 ...
 *
 * CLIENT-SAFE: Layer 2 (LLM classification) is replaced by direct
 * deterministic correction when Layer 1 detects boundary without repair path.
*/
...
export async function applyK05CrossModuleOverride(input: K05OverrideInput): Promise<K05OverrideResult> {
  ...
  // Client-safe: Layer 1 detected boundary without repair path → apply deterministic correction
