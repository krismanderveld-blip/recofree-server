...
import { estimateTokenCost, getModelTierFromModel } from '@/lib/ai/debug/token-cost-tracker';
...
export async function processMessage(
  ...
  const aiMsg: ChatMessage = {
    ...
    clinicalInfo: {
      ...
      deepAnalysis: await (async () => { try { const AsyncStorageModule = await import("@react-native-async-storage/async-storage"); const AS = AsyncStorageModule.default; const reportJson = await AS.getItem("@recofree_last_deep_analysis_report"); if (!reportJson) return "never_run"; const r = JSON.parse(reportJson); if (!r.ok) return `FAILED at=${r.timestamp?.slice(0,16)} err=${(r.error || "unknown").slice(0,80)}`; const storedS = r.totalSchemas ?? (Array.isArray(currentUserDat?.schemas) ? currentUserDat.schemas.length : 0); const storedM = r.totalModes ?? (Array.isArray(currentUserDat?.modes) ? currentUserDat.modes.length : 0); const storedT = r.totalTriggers ?? (Array.isArray(currentUserDat?.triggers) ? currentUserDat.triggers.length : 0); const reason = r.sectionsSkipped > 0 && r.sectionsAnalyzed === 0 ? 'hash_unchanged' : r.sectionsAnalyzed > 0 ? 'content_changed' : 'unknown'; return `run: analyzed=${r.sectionsAnalyzed} skipped=${r.sectionsSkipped} failures=${r.failures} | stored: schemas=${storedS} modes=${storedM} triggers=${storedT} | reason=${reason} at=${r.timestamp?.slice(0,16)}${r.failureDetails?.length ? " | errors=[" + r.failureDetails.map((d: any) => d.sectionId + ":" + d.error).join(", ") + "]" : ""}`; } catch { return "read_error"; } })(),
